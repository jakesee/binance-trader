...
export class Trader {
     ...
     * Private variables
     */
    private _binance: any;
    private MACD = technical.MACD;
    private BB = technical.BollingerBands;
    private RSI = technical.RSI;
    private EMA = technical.EMA;

    constructor(private binance: any) {
            ...
            if (price > stop && price <= bag.bid0) {
                ...
                if ((ask / bid) - 1 < symbol.config.strategy.buy.maxBuySpread) { // prevent pump
                    ...
                    if (!symbol.canBuy(quantity, bid)) {
                        bag.position = POSITIONS.SELL; // just concentrate on selling, the loss is too little
                        return;
                    }
    ...
    _bidding(symbol, price) {
        var bag = symbol.config.bag;
        var order = bag.order;
        if (order != null && bag.position == POSITIONS.BIDDING) {
            var book = symbol.getBook();
    ...
    _asking(symbol, price) {
        var bag = symbol.config.bag;
        var order = bag.order;
        if (order != null && bag.position == POSITIONS.ASKING) {
            var book = symbol.getBook();
            var bid0 = book.bids[0];
            var ask0 = book.asks[0];
    ...
    _sell(symbol, price) {
        ...
        if (shouldSell) {
            ...
            symbol.config.bag.position = POSITIONS.SELLING;
