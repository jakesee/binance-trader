...
class Binance {
    ...
    run() {
        ...
        var portfolio = _.reduce(this._portfolio, (o, p) => {
            o[p.asset + config.quote] = p;
            return o;
        }, {});
        _.each(symbols, (symbol) => {
            this._symbols[symbol] = new TSymbol(symbol, config[symbol]);
        ...
        return new Promise((resolve, reject) => {
            rest.account((err, data) => {
                ...
                if (err) {
                    ...
                    var balances = _.filter(data.balances, (b) => {
                        return symbols.includes(b.asset + config.quote);
                    });
                    _.each(balances, (b) => {
                        if (b.asset == config.quote) {
                            return true; // skip BTC
                        }
                        ...
                        if (b.asset != 'BNB') {
                            ...
                            if (!Array.isArray(tradeBNB))
                                tradeBNB = []; // some coins does not have BNB pairs
                        }
            ...
            */
            rest.klines({
                ...
                if (err) {
                    ...
                    this._symbols[symbol].kline = _.filter(data, (d) => {
                        return d != undefined;
                    });
    ...
    _loadTickerStream(symbols) {
        ...
        var assets = _.map(symbols, (symbol) => {
    ...
    _loadKlineStream(symbols) {
        const streams = this._binanceWS.streams;
        var assets = _.map(symbols, (symbol) => {
            return streams.kline(symbol, this._symbols[symbol].config.indicator.kline.interval);
        });
        this._binanceWS.onCombinedStream(assets, (streamEvent) => {
            ...
            if (symbol in this._symbols && this._symbols[symbol].ready()) {
                ...
                if (lastKline.openTime == thisKline.openTime) {
                    ...
                    if (this._symbols[symbol].kline.length >= 500)
                        this._symbols[symbol].kline.shift();
    ...
    _loadUserDataStream() {
        this._binanceWS.onUserData(rest, (streamData) => {
            ...
            if (!(this._symbols.hasOwnProperty(streamData.symbol)))
                return;
